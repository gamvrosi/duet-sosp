...
# $Id: DbTDistr.pm,v 1.5 2003/05/23 04:15:53 johnh Exp $
#
...
DbTDistr - t-distributions for stats
...
sub init_t_distr {
    my($t_values) = "
alphas: 0.4, 0.3, 0.2, 0.1, 0.06666666666667, 0.05, 0.04, 0.03333333333333, 0.025, 0.02, 0.01666666666667, 0.0125, 0.01, 0.00833333333333, .00625, 0.005
...
2147483647: 0.253, 0.524, 0.842, 1.282, 1.501, 1.645, 1.751, 1.834, 1.960, 2.054, 2.127, 2.241, 2.326, 2.395, 2.501, 2.57
";
    my($nui) = -1;
    foreach (split(/\n/, $t_values)) {
	next if (/^\s*$/);
	...
	if ($nui >= 0) {
	    push (@t_nus, $nu);
	    die("Bad number of values in table.\n") if ($#values != $#t_gammas);
	};
	foreach $i (0..$#values) {
	    die("Zero t_value for nu=$nu, gamma=$gamma.\n") if ($values[$i] == 0);
	    if ($nui >= 0) {
	        $t_values{$nu,$t_gammas[$i]} = $values[$i] + 0.0;
	    } else {
		$t_gammas[$i] = 1.0 - $values[$i];
	    };
	};
	$nui++;
    };
...
=head2 t_distr($nu, $gamma)

Return the value interpolated from a t-distribution.

=cut
sub t_distr {
    my($nu, $gamma) = @_;   # degrees of freedom, confidence half-interval
    ...
    croak("Gamma value too low, off of bottom of table.")
	if ($gamma < $t_gammas[0]);
    my($gammai);
    for ($gammai = 0; $gammai <= $#t_gammas; $gammai++) {
        last if ($t_gammas[$gammai] >= $gamma);
    };
    croak("Gamma value too high, off of top of table.")
        if ($gammai > $#t_gammas);

    my($nu_less) = $t_nus[$nui-1];
    my($nu_more) = $t_nus[$nui];
    my($gamma_less) = $t_gammas[$gammai-1];
    my($gamma_more) = $t_gammas[$gammai];
    if ($t_gammas[$gammai] == $gamma) {
	if ($t_nus[$nui] == $nu) {
	    return $t_values{$nu,$gamma};
	} else {
	    return &interpolate($nu, $nu_less, $nu_more, 
			$t_values{$nu_less,$gamma}, $t_values{$nu_more,$gamma});
	};
    } elsif ($t_nus[$nui] == $nu) {
        return &interpolate($gamma, $gamma_less, $gamma_more,
			$t_values{$nu,$gamma_less}, $t_values{$nu,$gamma_more});
    } else {
        croak("Neither nu nor gamma match, case not implemented.");
		# Geoff's code:
		# value_low = interpolate (gamma,
		#   gammas[gamma_index - 1],
		#   gammas[gamma_index],
		#   t_values[nu_index - 1].values[gamma_index - 1],
		#   t_values[nu_index - 1].values[gamma_index]);
		# value_high = interpolate (gamma,
		#   gammas[gamma_index - 1],
		#   gammas[gamma_index],
		#   t_values[nu_index].values[gamma_index - 1],
		#   t_values[nu_index].values[gamma_index]);
