...
static struct btrfs_device *find_device_by_bdev(struct btrfs_fs_info *fs_info,
						struct block_device *bdev)
{
	struct btrfs_device *device;
	struct btrfs_fs_devices *cur_devices;
...
#ifdef CONFIG_BTRFS_FS_REVMAP_DEBUG
		printk(KERN_DEBUG "iterate_range_item: l_offt = %llu\n",
							am_ctx->l_offt);
#endif /* CONFIG_BTRFS_FS_REVMAP_DEBUG */
...
again:
			...
			if (iterate)
				iterate(ekey, extent_start, extent_len);
...
int btrfs_iterate_items(struct btrfs_fs_info *fs_info,
			...
			struct btrfs_root *root, iterate_items_t iterate)
{
	struct btrfs_device *dev;
	u64 cur_offt = p_offt;
	u64 rem_len = len;
	...
	dev = find_device_by_bdev(fs_info, bdev);
	if (!dev) {
		printk(KERN_ERR "btrfs_iterate_items: device not found\n");
		ret = -ENODEV;
		goto out;
	}

#ifdef CONFIG_BTRFS_FS_REVMAP_DEBUG
	printk(KERN_DEBUG "btrfs_iterate_items: device id %llu, size %llu.\n",
	...
	while (rem_len) {
		...
		if (ret) {
			printk(KERN_ERR "btrfs_iterate_items: chunk not found\n");
			goto out;
		}

#ifdef CONFIG_BTRFS_FS_REVMAP_DEBUG
		printk(KERN_DEBUG "btrfs_iterate_items: chunk p_start %llu, "
			"l_start %llu, len %llu\n",
		...
		ret = find_chunk_addr_mapping(dev, de_ctx, cur_offt, rem_len,
								&am_ctx);
		if (ret) {
			printk(KERN_ERR "btrfs_iterate_items: address mapping "
				"info collection failed\n");
			goto out;
		}

#ifdef CONFIG_BTRFS_FS_REVMAP_DEBUG
		printk(KERN_DEBUG "btrfs_iterate_items: address mapping found\n"
				"btrfs_iterate_items: \tl_start %llu\n"
				"btrfs_iterate_items: \tl_offt %llu\n"
				"btrfs_iterate_items: \tl_offt_end %llu\n"
				"btrfs_iterate_items: \tl_end %llu\n"
				"btrfs_iterate_items: \tp_start %llu\n"
				"btrfs_iterate_items: \tp_offt %llu\n"
				"btrfs_iterate_items: \tstripe_len %llu\n"
				"btrfs_iterate_items: \tincrement %llu\n"
				"btrfs_iterate_items: \tnstripes %llu\n"
				"btrfs_iterate_items: \tnum_bytes %llu\n",
		...
		if (ret) {
			printk(KERN_DEBUG "btrfs_iterate_items: item iteration failed\n");
			goto out;
		}
